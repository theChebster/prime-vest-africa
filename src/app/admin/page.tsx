...
import { Users, Landmark, ArrowUpCircle, ArrowDownCircle, ShieldAlert, UserCheck } from "lucide-react";
// Since the file is in src/app/admin/page.tsx, we go up 2 levels to reach src
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 p-8 rounded-[40px]">
            <h3 className="text-lg font-black uppercase mb-6 flex items-center gap-2">
              <ShieldAlert size={20} className="text-yellow-500" /> Control Panel
            </h3>
            <div className="space-y-4">
              <AdminLink href="/admin/deposits" label="Review Deposit Proofs" count={stats.pendingDeposits} color="bg-green-500" />
              <AdminLink href="/admin/withdrawals" label="Process Payouts" count={stats.pendingWithdrawals} color="bg-red-500" />
              <AdminLink href="/admin/users" label="Manage User Database" count={null} color="bg-blue-500" />
            </div>
          </div>

          {/* Security Status */}
          <div className="bg-zinc-900/30 border border-zinc-800 p-8 rounded-[40px] flex flex-col justify-center">
             <UserCheck className="text-yellow-500 mb-4" size={32} />
             <h4 className="font-bold text-yellow-500 uppercase text-sm italic">Status: Secure</h4>
             <p className="text-zinc-500 text-[10px] mt-2 leading-relaxed uppercase font-black tracking-widest">
...
function StatCard({ title, value, icon }: any) {
  return (
    <div className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-[32px]">
      <div className="p-3 bg-black w-fit rounded-2xl border border-zinc-800 mb-4">{icon}</div>
...
function AdminLink({ href, label, count, color }: any) {
  return (
    <a href={href} className="flex justify-between items-center p-5 bg-black border border-zinc-800 rounded-2xl hover:bg-zinc-800 transition-all">
      ...
      {count !== null && count > 0 && (
        <span className={`${color} text-black text-[9px] font-black px-2 py-0.5 rounded uppercase`}>
          {count} NEW
        </span>
      )}
    </a>
